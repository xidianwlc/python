...
class Searcher():
    def __init__(self, cookie, keyword):
        ...
        self.url = "https://s.taobao.com/search?q=%s&sort=sale-desc" % keyword
...
class Cookier():
    ...
    async def cookie_main(self):
        ...
        await page.setExtraHTTPHeaders({'Cookie': self.cookies})
        # await page.reload()
        await page.goto(self.url)  # 访问登录页面
        # 替换淘宝在检测浏览时采集的一些参数。
        ...
        if slider:
            ...
            await page.reload()
            self.rtn_cookies = await self.get_cookies(
                page)  # 导出cookie 完成登陆后就可以拿着cookie玩各种各样的事情了。
    ...
    async def get_cookies(self, page):
        ...
        for cookie in cookies_list:
            ...
            cookies += str_cookie
        rtn_cookies = self.cookies + "; " + cookies
        print(cookies)
        return rtn_cookies
    ...
    async def mouse_slide(self, page=None):
        ...
        await page.mouse.move(
            2000, 2000,
            {'delay': random.randint(1000, 2000)})  # 不同场景的验证码滑动长度和方向可能不同同。
        await page.mouse.up()
        return page
...
if __name__ == "__main__":
    ...
    with open('sec_data/cookies.txt', 'r') as c:
        cookie = c.readline()
    for i in range(1):
        tb_searcher = Searcher(cookie, keyword)
        goods = tb_searcher.get_data()
        print(goods)
        print(len(goods))
    # i = 0
    # for g in goods:
